...
void MorphHandler::Revert()
{
	m_valueMap.clear();
	m_sculptStorage.clear();
}

void MorphHandler::RevertInternals()
{
	m_internalMap.clear();
}
...
bool TRIFile::Apply(NiGeometry * geometry, BSFixedString morphName, float relative)
{
	...
	virtual bool Accept(BGSHeadPart * headPart)
	{
		...
		if (it != m_partList->end()) { // Found part tri file
			...
			if(targetPart) {
				...
				g_morphHandler.GetModelTri(sourceMorphPath, sourceData);
				g_morphHandler.GetModelTri(targetMorphPath, targetData);
				...
				if (sourceData.vertexCount == targetData.vertexCount && sourceData.vertexCount > 0 && targetData.vertexCount > 0) {
					...
					auto titer = g_morphHandler.m_morphMap.find(targetMorphPath);
					if(titer != g_morphHandler.m_morphMap.end())
						g_morphHandler.m_morphMap.emplace(sourceMorphPath, titer->second);
					if(headPart->type == BGSHeadPart::kTypeFace)
						m_acceptDefault = true;
...
SliderInternalPtr MorphHandler::GetSliderByIndex(TESRace * race, UInt32 index)
{
...
float ValueMap::GetMorphValueByName(TESNPC* npc, BSFixedString name)
{
...
void ValueMap::SetMorphValue(TESNPC* npc, BSFixedString name, float value)
{
	ValueMap::iterator it = find(npc);
	if(it != end()) {
		...
		ValueSet newSet;
		newSet.emplace(name, value);
		emplace(npc, newSet);
	}
}

void ValueSet::SetValue(BSFixedString name, float value)
{
	ValueSet::iterator val = find(name);
	if (val != end())
		val->second = value;
	else
		emplace(name, value);
...
void ValueSet::ClearValue(BSFixedString name)
{
	ValueSet::iterator val = find(name);
	if(val != end())
		erase(val);
...
float ValueSet::GetValue(BSFixedString name)
{
	ValueSet::iterator val = find(name);
	if(val != end())
		return val->second;
...
float MorphHandler::GetMorphValueByName(TESNPC* npc, BSFixedString name)
{
	return m_valueMap.GetMorphValueByName(npc, name);
}

void MorphHandler::SetMorphValue(TESNPC* npc, BSFixedString name, float value)
{
...
bool MorphHandler::CacheHeadPartModel(BGSHeadPart * headPart, bool cacheTRI)
{
...
bool MorphHandler::GetModelTri(BSFixedString filePath, TRIModelData & modelData)
{
...
TRIModelData & MorphHandler::GetExtendedModelTri(BSFixedString morphName, bool cacheTRI)
{
	std::string filePath(SLIDER_DIRECTORY);
	filePath.append(morphName.data);
	BSFixedString morphFile(filePath.c_str());
	ModelMap::iterator it = m_modelMap.find(morphFile);
	if(it == m_modelMap.end()) {
		void* memory = FormHeap_Allocate(sizeof(TESModelTri));
		memset(memory, 0, sizeof(TESModelTri));
		((UInt32*)memory)[0] = 0x010A2E08;
		TESModelTri* xData = (TESModelTri*)memory;
		xData->SetModelName(morphFile.data);

		TRIModelData data;
		data.morphModel = xData;
		if (!cacheTRI) {
			data.vertexCount = ReadTRIVertexCount(morphFile.data);
		}
		else {
			data.triFile = std::make_shared<TRIFile>();
			data.triFile->Load(morphFile.data);
			data.vertexCount = data.triFile->vertexCount;
		}
		...
		it->second.triFile = std::make_shared<TRIFile>();
		it->second.triFile->Load(morphFile.data);
	}
	
	return it->second;
}

PresetDataPtr MorphHandler::GetPreset(TESNPC* npc)
{
...
void MorphHandler::AssignPreset(TESNPC * npc, PresetDataPtr presetData)
{
	ErasePreset(npc);
	m_mappedPreset.emplace(npc, presetData);
}

bool MorphHandler::ErasePreset(TESNPC * npc)
{
	auto it = m_mappedPreset.find(npc);
	if (it != m_mappedPreset.end()) {
...
void MorphHandler::ClearPresets()
{
	m_mappedPreset.clear();
}

void MorphHandler::ApplyPreset(TESNPC * npc, BSFaceGenNiNode * faceNode, BGSHeadPart * headPart)
{
					...
					if (shaderProperty->GetRTTI() == NiRTTI_BSLightingShaderProperty) {
						...
						if (headPart->type == BGSHeadPart::kTypeFace) {
							for (auto & texture : presetData->faceTextures)
								material->textureSet->SetTexturePath(texture.index, texture.name.data);
...
void MorphHandler::ApplyPresetData(Actor * actor, PresetDataPtr presetData, bool setSkinColor, ApplyTypes applyType)
{
	...
	if (chargenData) {
		...
		if (headParts && headPartList) {
			FormHeap_Free(headParts);
			npc->numHeadParts = headPartList->count;
			headParts = (BGSHeadPart **)FormHeap_Allocate(npc->numHeadParts * sizeof(BGSHeadPart*));
	...
	for (auto & morph : presetData->customMorphs)
		g_morphHandler.SetMorphValue(npc, morph.name, morph.value);
...
bool MorphMap::Visit(BSFixedString key, Visitor & visitor)
{
...
bool MorphHandler::VisitMorphMap(BSFixedString key, MorphMap::Visitor & visitor)
{
	//key = toLower(key);
	return m_morphMap.Visit(key, visitor);
}

void MorphMap::AddMorph(BSFixedString key, BSFixedString value)
{
	//key = toLower(key);
	MorphMap::iterator it = find(key);
	if(it != end()) {
...
void MorphHandler::ReadMorphs(std::string fixedPath, std::string modName, std::string fileName)
{
...
#endif
			m_morphMap.AddMorph(BSFixedString(key.c_str()), BSFixedString(params[i].c_str()));
		}
	}
}

void MorphHandler::ReadRaces(std::string fixedPath, std::string modPath, std::string fileName)
{
...
void SliderMap::AddSlider(BSFixedString key, UInt8 gender, SliderInternal & sliderInternal)
{
...
SliderMapPtr MorphHandler::ReadSliders(std::string fixedPath, std::string modPath, std::string fileName)
{
	SliderMapPtr sliderMap = NULL;
	std::string fullPath = fixedPath + modPath + fileName;
	BSResourceNiBinaryStream file(fullPath.c_str());
	...
	while(BSReadLine(&file, &str))
	{
		...
		BSFixedString sliderName = BSFixedString(lSide.c_str());
		...
		switch(sliderInternal.type)
		{
			case SliderInternal::kTypeSlider:
			{
				...
				sliderInternal.lowerBound = BSFixedString(params[2].c_str());
				sliderInternal.upperBound = BSFixedString(params[3].c_str());

				if(sliderInternal.lowerBound == BSFixedString("None"))
					sliderInternal.lowerBound = BSFixedString("");
				if(sliderInternal.upperBound == BSFixedString("None"))
					sliderInternal.upperBound = BSFixedString("");
			}
			break;
...
SliderInternalPtr MorphHandler::GetSlider(TESRace * race, UInt8 gender, BSFixedString name)
{
...
{
	return s1->name.data < s2->name.data;
}

SliderList * MorphHandler::CreateSliderList(TESRace * race, UInt8 gender)
{
...
void MorphHandler::AddSlider(TESRace * race, SliderInternalPtr & slider)
{
	RaceSliders::iterator it = m_internalMap.find(race);
	if(it != m_internalMap.end()) {
...
void MorphHandler::ApplyMorph(TESNPC * npc, BGSHeadPart * headPart, BSFaceGenNiNode * faceNode)
{
		...
		if(slider) {
			if(slider->type == SliderInternal::kTypePreset) {
				if(it->second != 0) { // There should be no zero morph for presets
					...
					sprintf_s(buffer, MAX_PATH, "%s%d", slider->lowerBound.data, (UInt32)it->second);
...
void MorphHandler::ApplyMorphs(TESNPC * npc, BSFaceGenNiNode * faceNode)
{
	char buffer[MAX_PATH];
...
#endif
				SetMorph(npc, faceNode, morphName.data, relative);
			}
		}
	}
}

void MorphHandler::SetMorph(TESNPC * npc, BSFaceGenNiNode * faceNode, const char * name, float relative)
{
#ifdef _DEBUG_MORPH
	_DMESSAGE("Applying Morph %s", name);
#endif
	TESNPC::MorphAction morphAction;
	morphAction.faceNode = faceNode;
	morphAction.npc = npc;
	morphAction.value = relative;
	BSFixedString morphName(name);
	const char * morphNames[4];
	memset(morphNames, 0, 4 * 4);
	morphNames[0] = morphName.data;
	morphAction.morphName = morphNames;
	CALL_MEMBER_FN(npc, ApplyMorph)(&morphAction);
}

void MorphHandler::LoadSliders(SliderArray * sliderArray, RaceMenuSlider * slider)
{
	PlayerCharacter * player = (*g_thePlayer);
	...
	if(valueSet) {
		...
		while (it != valueSet->end()) {
			...
			if (!foundMorph) {
				_DMESSAGE("Erasing %s", it->first.data);
				valueSet->erase(it++);
			}
			else
				it++;
	...
	for(auto it = currentList->begin(); it != currentList->end(); ++it)
	{
		...
		sliderName.append(slider->name.data);
		...
		RaceMenuSlider newSlider(slider->category, morphName.data, "ChangeDoubleMorph", sliderId++, sliderIndex, 2, 1, lowerBound * lowerMultiplier, upperBound * upperMultiplier, value, interval, 0);
		newSlider.unk12C = 0;
		CALL_MEMBER_FN(sliderArray, AddSlider)(&newSlider);
...
void MorphHandler::Save(TESNPC* npc, SKSESerializationInterface * intfc, UInt32 kVersion)
{
	...
	if (sculptData) {
		if (sculptData->size() > 0) {
			...
			for (auto part : *sculptData) {
				if (part.first.data[0] != 0 && part.second->size() > 0) {
					numValidParts++;
				}
			}
			...
			if (numValidParts > 0) {
				for (auto part : *sculptData) {
					...
					if (diffCount > 0) {
						UInt16 length = strlen(part.first.data);
						intfc->OpenRecord('SCPT', kVersion);
						intfc->WriteRecordData(&length, sizeof(length));
						intfc->WriteRecordData(part.first.data, length);
	...
	if (valueSet) {
		...
		if (numMorphs > 0) {
			...
			for (auto it = valueSet->begin(); it != valueSet->end(); ++it) {
				if (it->second != 0.0) {
					UInt16 length = strlen(it->first.data);
					intfc->OpenRecord('MRPH', kVersion);
					intfc->WriteRecordData(&length, sizeof(length));
					intfc->WriteRecordData(it->first.data, length);
...
bool MorphHandler::SaveJsonPreset(const char * filePath)
{
	...
	if (g_bodyMorphInterface) {
		g_bodyMorphInterface->VisitMorphs(player, [&](BSFixedString name, std::unordered_map<BSFixedString, float> * map)
		{
			for (auto & it : *map)
			{
...
bool MorphHandler::SaveBinaryPreset(const char * filePath)
{
	IFileStream		currentFile;
	IFileStream::MakeAllDirs(filePath);
	...
	try
	{
		...
		fileHeader.runtimeVersion =	RUNTIME_VERSION_1_9_32_0;
		...
		if(valueSet)
		{
			...
			{
				if(it->second != 0.0) {
					UInt16 strLen = strlen(it->first.data);
					currentFile.Write16(strLen);
					currentFile.WriteBuf(it->first.data, strLen);
...
bool MorphHandler::LoadJsonPreset(const char * filePath, PresetDataPtr presetData)
{
	...
	if (!bodyMorphs.empty()) {
		for (auto & bm : bodyMorphs) {
			...
			{
				for (auto & jvalue : values) {
					BSFixedString key = jvalue["key"].asString().c_str();
...
bool MorphHandler::LoadBinaryPreset(const char * filePath, PresetDataPtr presetData)
{
	bool loadError = false;
	BSResourceNiBinaryStream file(filePath);
	if (!file.IsValid()) {
...
BSFixedString SculptData::GetHostByPart(BGSHeadPart * headPart)
{
...
MappedSculptDataPtr SculptData::GetSculptHost(BSFixedString host, bool create)
{
	auto it = find(host);
	if (it != end())
		return it->second;
...
void SKSETaskApplyMorphs::Run()
{
	...
	if (faceNode) {
		g_morphHandler.ApplyMorphs(actorBase, faceNode);
...
void SKSETaskImportHead::Run()
{
	...
	std::unordered_map<BSFixedString, std::tuple<BGSHeadPart*,NiGeometry*, NiGeometry*>> typeMap;
	for (UInt32 i = 0; i < numParts; i++)
	{
		BGSHeadPart * headPart = headParts[i];
			...
			if (rootNode)
			{
				VisitObjects(rootNode, [&](NiAVObject* trishape)
				{
					...
					if (parent && BSFixedString(parent->m_name) == BSFixedString("BSFaceGenNiNodeSkinned")) {
						NiGeometry * geometry = trishape->GetAsNiGeometry();
						if (geometry) {
							NiGeometryData * geometryData = niptr_cast<NiGeometryData>(geometry->m_spModelData);
							if (geometryData) {
	...
	for (auto obj : typeMap) {
		...
		NiGeometry * source = std::get<1>(obj.second);
		NiGeometry * target = std::get<2>(obj.second);
