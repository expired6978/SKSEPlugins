...
public:
	...
	virtual void InstallOverlay(const char * nodeName, const char * path, TESObjectREFR * refr, NiGeometry * source, NiNode * destination, BGSTextureSet * textureSet = NULL);
	virtual void ResetOverlay(const char * nodeName, TESObjectREFR * refr, NiGeometry * source, NiNode * destination, BGSTextureSet * textureSet = NULL, bool resetDiffuse = false); // Re-applies the skin's textures

	virtual BSFixedString GetDefaultTexture();
	virtual void SetDefaultTexture(BSFixedString newTexture);

	// Relinks an overlay node by name to the new source
	virtual void RelinkOverlay(const char * nodeName, TESObjectREFR * refr, NiGeometry * source, NiNode * skeleton);
